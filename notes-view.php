...
function prevItem() {
    ...
    window.location.href = `notes-view.php?id=${chapterId}&itemId=${prevItem.id}&parentId=${parentId}`;
}

function nextItem() {
    if (currentIndex >= items.length - 1) return;
    const nextItem = items[currentIndex + 1];
    window.location.href = `notes-view.php?id=${chapterId}&itemId=${nextItem.id}&parentId=${parentId}`;
}
...
function toggleBookmark() {
    ...
    fetch('/bookmark-proxy.php?module=notes', {
        method: 'POST',
        body: formData
    })
