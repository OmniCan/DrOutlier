...
function NewExamCasesCategories() {
    ...
    useEffect(() => {
        ...
        // Check cache first
        const cacheKey = `new-exam-cases-categories-${user}`;
        const cachedData = ApiCache.get(cacheKey);
        
        if (cachedData) {
            console.log('Using cached New Exam Cases categories');
        ...
        console.log('Fetching New Exam Cases categories from:', `${baseUrl}/api/new-exam-cases/categories`);
        
        axios.post(`${baseUrl}/api/new-exam-cases/categories`, {}, {
            headers: {
                'Authorization': `Bearer ${cookies}`
            }
        }).then((response) => {
            console.log('New Exam Cases Categories API Response:', response);
            ...
            console.error('Error fetching New Exam Cases categories:', error);
                                        ...
                                        {categories && categories.length > 0 ? (
                                            categories?.map((category, index) => (
                                                <div className="col-md-4 col-sm-6 col-6" key={category.id} style={{ padding: '18px', flex: '0 0 20%', maxWidth: '20%' }}>
                                                    <Link href={`/new-exam-cases/chapters?id=${category.id}`}>
...
export default NewExamCasesCategories
