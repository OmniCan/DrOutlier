...
function TheoryNotesCategories() {
    ...
    useEffect(() => {
        ...
        // Check cache first
        const cacheKey = `theory-notes-categories-${user}`;
        const cachedData = ApiCache.get(cacheKey);
        
        if (cachedData) {
            console.log('Using cached Theory Notes categories');
        ...
        console.log('Fetching Theory Notes categories from:', `${baseUrl}/api/theory-notes/categories`);
        
        axios.post(`${baseUrl}/api/theory-notes/categories`, {}, {
            headers: {
                'Authorization': `Bearer ${cookies}`
            }
        }).then((response) => {
            console.log('Theory Notes Categories API Response:', response);
            ...
            console.error('Error fetching Theory Notes categories:', error);
                                        ...
                                        {categories && categories.length > 0 ? (
                                            categories?.map((category, index) => (
                                                <div className="col-md-4 col-sm-6 col-6" key={category.id} style={{ padding: '18px', flex: '0 0 20%', maxWidth: '20%' }}>
                                                    <Link href={`/theory-notes/chapters?id=${category.id}`}>
...
export default TheoryNotesCategories
