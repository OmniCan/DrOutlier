                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                        {items && items.length > 0 ? (
                                            items?.map((item, index) => (
                                                <Link key={item.id} href={`/new-spotters/view?id=${categoryId}&itemId=${item.id}${parentCategoryId ? `&parentId=${parentCategoryId}` : ''}#page${index + 1}`} style={{ textDecoration: 'none' }}>
                                        ...
                                        <div className="col-12">
                                            <div style={{ 
                                                ...
                                                {currentChapterIndex > 0 ? (
                                                    <Link href={`/new-spotters/category?id=${allChapters[currentChapterIndex - 1].id}${parentCategoryId ? `&parentId=${parentCategoryId}` : ''}`}>
                                                        <button style={{ 
                                                            padding: '15px 30px',
                                                            background: 'linear-gradient(150deg, #44A6C5 0%, #1E4FFD 100%)',
                                                            borderRadius: '12px',
                                    ...
                                    {items && items.length > 0 ? (
                                        items?.map((item, index) => (
                                            <div className="col-6" key={item.id}>
                                                <Link href={`/new-spotters/view?id=${categoryId}&itemId=${item.id}${parentCategoryId ? `&parentId=${parentCategoryId}` : ''}#page${index + 1}`}>
...
export default NewSpottersCategory
