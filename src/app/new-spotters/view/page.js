...
function NewSpotterViewer() {
    ...
    useEffect(() => {
        ...
        // Fetch items list
        axios.post(`${baseUrl}/api/new-spotters/items-by-chapter`, {
            ...
            if (itemsList.length > 0) {
                ...
                if (itemsList[0].pdf_file) {
                    const pdfPath = `${baseUrl}/assets/admin/images/new_spotters_pdf/${itemsList[0].pdf_file}`;
            ...
            if (itemId && itemsList.length > 0) {
                ...
                if (itemIndex !== -1) {
                    ...
                    if (itemsList[itemIndex].pdf_file) {
                        const pdfPath = `${baseUrl}/assets/admin/images/new_spotters_pdf/${itemsList[itemIndex].pdf_file}`;
                        const proxyUrl = `/api/proxy-pdf?url=${encodeURIComponent(pdfPath)}`;
                        setPdfUrl(proxyUrl);
                    }
                }
            ...
            if (parentCategoryId) {
                ...
                // No parent ID - try to get all new-spotters categories
                axios.get(`${baseUrl}/api/new-spotters/categories`, {
                    headers: {
                        'Authorization': `Bearer ${cookies}`
                    }
    ...
    const saveBookmark = (itemId) => {
        ...
        axios.post(`${baseUrl}/api/new-spotters/toggle-bookmark`, formData, {
            headers: {
                'Authorization': `Bearer ${cookies}`,
            }
        })
    ...
    useEffect(() => {
        if (currentItem && currentItem.pdf_file) {
            const pdfPath = `${baseUrl}/assets/admin/images/new_spotters_pdf/${currentItem.pdf_file}`;
                                    ...
                                    {allChapters.length > 0 && (
                                        ...
                                        >
                                            ...
                                            {currentChapterIndex > 0 ? (
                                                <Link href={`/new-spotters/category?id=${allChapters[currentChapterIndex - 1].id}${parentId ? `&parentId=${parentId}` : ''}`}>
                                                    <button style={{ 
                                                        padding: '12px 24px',
                        ...
                        <div className="container" style={{ maxWidth: '1140px', marginTop: '40px' }}>
                            <div style={{ background: 'white', borderRadius: '24px', padding: '60px' }}>
                                <div style={{ textAlign: 'center' }}>
                                    ...
                                    <Link href={`/new-spotters/category?id=${categoryId}${parentId ? `&parentId=${parentId}` : ''}`} style={{
                                        padding: '12px 30px',
...
export default NewSpotterViewer
