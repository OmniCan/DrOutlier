...
function page() {
    ...
    const [bookmarkedBasics, setBookmarkedBasics] = useState({});
    ...
    const totalPages = Math?.ceil(basicDetails?.length / itemsPerPage);

    const currentData = basicDetails?.slice(
    ...
    const saveBasic = (basicId) => {
        const cookies = Cookies.get('user-token');
        const formData = new FormData();
        formData.append('user_id', userid);
        formData.append('basic_id', basicId);

        axios.post(`${baseUrl}/api/basic-category/change-basic-bookmark-status`, formData, {
            headers: {
            ...
            .then((response) => {
                ...
                // Toggle the bookmark status for this specific basic
                setBookmarkedBasics(prev => ({
                    ...prev,
                    [basicId]: !prev[basicId]
                }));
            })
            .catch((error) => {
    ...
    useEffect(() => {
        ...
        // Fetch basics list
        axios.post(`${baseUrl}/api/basic-category/category-basic?page=1&category=${categoryId}`, {}, {
            headers: {
                'Authorization': `Bearer ${cookies}`
            }
        }).then((categoryResponse) => {
            const basicsList = categoryResponse?.data?.data?.notes?.data || [];
            setBasicDetails(basicsList);
            
            // Get category info
            let categoryInfo = null;
            if (basicsList.length > 0 && basicsList[0].category) {
                categoryInfo = basicsList[0].category;
                setCategoryName(categoryInfo.name || 'Practical Essentials');
            } else {
                setCategoryName('Practical Essentials');
            }
            
            console.log('Category Info from basics:', categoryInfo);
            console.log('Has parent_id?', categoryInfo?.parent_id);
            
            // Fetch bookmarked basics
            const bookmarkFormData = new FormData();
            bookmarkFormData.append("user_id", userid);
            
            axios.post(`${baseUrl}/api/basic-category/get-basic-bookmark`, bookmarkFormData, {
                headers: {
                    'Authorization': `Bearer ${cookies}`
                }
            }).then((bookmarkResponse) => {
                const savedBasics = bookmarkResponse?.data?.data?.list?.data || [];
                const bookmarksMap = {};
                savedBasics.forEach((basic) => {
                    bookmarksMap[basic.id] = true;
                });
                setBookmarkedBasics(bookmarksMap);
            }).catch((error) => {
            ...
            // If basicId is provided in URL, find its index and set as current page
            if (basicId) {
                const index = basicsList.findIndex(basic => basic.id.toString() === basicId.toString());
                if (index !== -1) {
                                        ...
                                        <div style={{ 
                                                    ...
                                                    >
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path 
                                                                ...
                                                                fill={bookmarkedBasics[e.id] ? 'url(#bookmarkGradient)' : '#1E1E1E'}
                                                            />
                                                            <defs>
                                            ...
                                            >
                                                ...
                                                {currentChapterIndex > 0 ? (
                                                    <Link href={`/practical-essentials/category?id=${allChapters[currentChapterIndex - 1].id}`}>
                                                        <button style={{ 
                                                            padding: '15px 30px',
