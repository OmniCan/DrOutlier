...
function NewTableVivaCategories() {
    ...
    useEffect(() => {
        ...
        // Check cache first
        const cacheKey = `new-table-viva-categories-${user}`;
        const cachedData = ApiCache.get(cacheKey);
        
        if (cachedData) {
            console.log('Using cached New Table Viva categories');
        ...
        console.log('Fetching New Table Viva categories from:', `${baseUrl}/api/new-table-viva/categories`);
        
        axios.post(`${baseUrl}/api/new-table-viva/categories`, {}, {
            headers: {
                'Authorization': `Bearer ${cookies}`
            }
        }).then((response) => {
            console.log('New Table Viva Categories API Response:', response);
            ...
            console.error('Error fetching New Table Viva categories:', error);
                                        ...
                                        {categories && categories.length > 0 ? (
                                            categories?.map((category, index) => (
                                                <div className="col-md-4 col-sm-6 col-6" key={category.id} style={{ padding: '18px', flex: '0 0 20%', maxWidth: '20%' }}>
                                                    <Link href={`/new-table-viva/chapters?id=${category.id}`}>
...
export default NewTableVivaCategories
