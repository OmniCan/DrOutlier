...
function prevItem() {
    ...
    window.location.href = `osce-view.php?id=${chapterId}&itemId=${items[currentIndex - 1].id}&parentId=${parentId}`;
}
...
function toggleBookmark() {
    ...
    fetch('/bookmark-proxy.php?module=osce', {
        method: 'POST',
        body: formData
    })
...
function refreshBookmarkState() {
    ...
    fetch('/bookmark-proxy.php?module=osce&action=status', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
if (items.length > 0) { loadPDF(); refreshBookmarkState(); }
</script>
