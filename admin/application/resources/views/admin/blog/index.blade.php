                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" class="col-xs-1 sort_order width1 text-center"
                                                data-id="{{ $blog->id }}" placeholder=""
                                                value="{{ $blog->sort_order ?? '' }}" style="width:60px;">
                                        </td>
                                        <td>
                                            <a title="@lang('Edit')"
                                                href="{{ route('admin.blogs.blog-edit', $blog->id) }}"
                                                class="btn btn-sm btn--primary"> <i class="la la-pen"></i></a>

                                            <button class="btn btn-outline--danger btn-sm confirmationBtn"
                                                data-question="@lang('Are you sure that you want to delete?')"
                                                data-action="{{ route('admin.blogs.blog-delete', $blog->id) }}"
                                                title="Delete"><i class="las la-trash"></i></button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.blogs.blog-create') }}" class="btn btn-sm btn--primary box--shadow1 text--small"><i
            class="las la-angle-double-left"></i>@lang('Add Notes')</a>
@endpush
...
@push('script')
    <script>
        ...
        $(".sort_order").on("blur", function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.blogs.update-sort-order') }}",
                data: {
                    status_id: status_id,
