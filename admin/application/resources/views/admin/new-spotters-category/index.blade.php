                        ...
                        <tbody>
                            @forelse($datalist as $k=>$data)
                                <tr>
                                    ...
                                    <td>
                                        <a title="@lang('Edit')" href="{{ route('admin.new-spotters-category.edit', $data->id)}}" class="btn btn-sm btn--primary" > <i class="la la-pen"></i></a>

                                        <button class="btn btn-outline--danger btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to delete?')" data-action="{{ route('admin.new-spotters-category.delete', $data->id)}}" title="Delete"><i class="las la-trash"></i></button>
                                    </td>
                                </tr>
                                @foreach($data->child as $key=>$child)
                                    <tr>
                                        ...
                                        <td>
                                            <a title="@lang('Edit')" href="{{ route('admin.new-spotters-category.edit', $child->id)}}" class="btn btn-sm btn--primary" > <i class="la la-pen"></i></a>

                                            <button class="btn btn-outline--danger btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to delete?')" data-action="{{ route('admin.new-spotters-category.delete', $child->id)}}" title="Delete"><i class="las la-trash"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            @empty
...
<a href="{{route('admin.new-spotters-category.create')}}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="las la-angle-double-left"></i>@lang('Add Category')</a>
@endpush
...
<script>
    (function ($) {
        ...
        $('.premium-badge').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('admin.new-spotters-category.toggle-premium', '') }}/" + id,
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}"
                },
