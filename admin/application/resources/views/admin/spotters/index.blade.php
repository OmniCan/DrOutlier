                    ...
                    <table class="table table--light style--two ">
                        ...
                        <tbody>
                            @forelse($spotterslist as $spotters)
                                <tr>
                                    <td>
                                        <img src="{{getImage(getFilePath('SpottersImage').'/'.@$spotters->image)}}" height="50" width="50"/>
                                    </td>
                                    <td>
                                        @php
                                            ...
                                            if($category) {
                                                if($category->parent_id != 0) {
                                                    ...
                                                    $parentCategory = \App\Models\SpottersCategory::find($category->parent_id);
                                    ...
                                    <td>
                                        <a title="@lang('Edit')" href="{{ route('admin.spotters.spotters-edit', $spotters->id)}}" class="btn btn-sm btn--primary" > <i class="la la-pen"></i></a>

                                        <button class="btn btn-outline--danger btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to delete?')" data-action="{{ route('admin.spotters.spotters-delete', $spotters->id)}}" title="Delete"><i class="las la-trash"></i></button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
...
<a href="{{route('admin.spotters.spotters-create')}}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="las la-angle-double-left"></i>@lang('Add Spotters')</a>
@endpush
...
<script>
  ...
  $(".sort_order").on("blur", function(e) {
      ...
      $.ajax({
          type: "POST",
          url: "{{ route('admin.spotters.update-sort-order') }}",
          data: {
              status_id: status_id,
