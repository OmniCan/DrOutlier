                    ...
                    <table class="table table--light style--two ">
                        ...
                        <tbody>
                            @forelse($spotterslist as $spotters)
                                <tr>
                                    <td>
                                        <img src="{{getImage(getFilePath('TheoryNotesImage').'/'.@$spotters->image)}}" height="50" width="50"/>
                                    </td>
                                    <td>
                                        @php
                                            ...
                                            if($category) {
                                                if($category->parent_id != 0) {
                                                    ...
                                                    $parentCategory = \App\Models\TheoryNotesCategory::find($category->parent_id);
                                    ...
                                    <td>
                                        @if($spotters->pdf_file)
                                            <a href="{{getImage(getFilePath('TheoryNotesPDF').'/'.@$spotters->pdf_file)}}" target="_blank" class="btn btn-sm btn--success">
                                    ...
                                    <td>
                                        <a title="@lang('Edit')" href="{{ route('admin.theory-notes.theory-notes-edit', $spotters->id)}}" class="btn btn-sm btn--primary" > <i class="la la-pen"></i></a>

                                        <button class="btn btn-outline--danger btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to delete?')" data-action="{{ route('admin.theory-notes.theory-notes-delete', $spotters->id)}}" title="Delete"><i class="las la-trash"></i></button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
...
<a href="{{route('admin.theory-notes.theory-notes-create')}}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="las la-angle-double-left"></i>@lang('Add New Theory Note')</a>
@endpush
...
<script>
  ...
  $(".sort_order").on("blur", function(e) {
      ...
      $.ajax({
          type: "POST",
          url: "{{ route('admin.theory-notes.update-sort-order') }}",
          data: {
              status_id: status_id,
  ...
  $('.premium-badge').on('click', function() {
      ...
      $.ajax({
          url: "{{ route('admin.theory-notes.toggle-premium', '') }}/" + id,
          type: 'POST',
          data: {
              _token: "{{ csrf_token() }}"
          },
