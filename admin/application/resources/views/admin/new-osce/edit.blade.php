...
<div class="row mb-none-30">
    <div class="col-lg-12 mb-30">
        <div class="card">
            <div class="card-body">
                <form action="{{route('admin.new-osce.new-osce-update' , $osce->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                                        ...
                                        @foreach ($parentCategories as $parent)
                                            ...
                                            @foreach ($category as $cate)
                                                @if($cate->parent_id == $parent->id)
                                                    <option value="{{$cate->id}}" {{($osce->category == $cate->id)?'selected':''}} style="padding-left: 20px;">
                                                        &nbsp;&nbsp;&nbsp;&nbsp;{{ $cate->name }}
                                                    </option>
                                                @endif
                                            @endforeach
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" id="title" value="{{$osce->title}}" class="form-control" placeholder="@lang('Enter Title')" required>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="number" name="sort_order" id="sort_order" value="{{$osce->sort_order}}" class="form-control" placeholder="@lang('Enter sort order')" required>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                @if($osce->image)
                                    <img src="{{getImage(getFilePath('NewOsceImage').'/'.@$osce->image)}}" width="100" height="100"/>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <textarea class="ckeditor form-control" name="description">{{$osce->description}}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="is_premium">
                                    <option value="0" {{ $osce->is_premium == 0 ? 'selected' : '' }}>Free</option>
                                    <option value="1" {{ $osce->is_premium == 1 ? 'selected' : '' }}>Premium</option>
                                </select>
                            </div>
                        </div>
...
<a href="{{route('admin.new-osce.new-osce-index')}}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="las la-angle-double-left"></i>@lang('Go Back')</a>
@endpush
