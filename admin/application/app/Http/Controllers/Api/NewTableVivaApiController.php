...
class NewTableVivaApiController extends Controller
{
    // Get all parent categories (categories with parent_id = 0)
    public function getCategories()
    {
        try {
            $categories = NewTableVivaCategory::where('parent_id', 0)
                ->where('status', 1)
                ->orderBy('name', 'ASC')
                ->get();
    ...
    public function getChapters(Request $request)
    {
        try {
            ...
            $parentCategory = NewTableVivaCategory::find($categoryId);
            $categoryName = $parentCategory ? $parentCategory->name : '';

            // Get all child categories (chapters)
            $chapters = NewTableVivaCategory::where('parent_id', $categoryId)
                ->where('status', 1)
                ->orderBy('name', 'ASC')
                ->get();
            ...
            $chapters = $chapters->map(function ($chapter) {
                $firstItem = NewTableViva::where('category', $chapter->id)
    ...
    public function getItemsByChapter(Request $request)
    {
        try {
            ...
            // Get chapter info
            $chapter = NewTableVivaCategory::find($chapterId);
            $chapterName = $chapter ? $chapter->name : '';

            // Get all items for this chapter, ordered by sort_order
            $items = NewTableViva::where('category', $chapterId)
                ->orderBy('sort_order', 'ASC')
                ->get();
    ...
    public function getItem(Request $request)
    {
        try {
            ...
            $item = NewTableViva::with('categories')->find($request->item_id);
