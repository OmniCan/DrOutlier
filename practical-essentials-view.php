<?php
/**
 * Practical Essentials - PDF Viewer Page
...
function loadPDF() {
    ...
    const pdfUrl = '/pdf-proxy.php?module=new_table_viva_pdf&file=' + encodeURIComponent(currentItem.pdf_file);
...
function prevItem() {
    ...
    window.location.href = `practical-essentials-view.php?id=${chapterId}&itemId=${items[currentIndex - 1].id}&parentId=${parentId}`;
}
...
function refreshBookmarkState() {
    ...
    fetch('/bookmark-proxy.php?module=practical-essentials&action=status', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
