...
if ($chapterId) {
    $response = apiRequest('/new-spotters/items-by-chapter', 'POST', ['chapter_id' => $chapterId], $token);
...
function prevItem() {
    ...
    window.location.href = `spotters-view.php?id=${chapterId}&itemId=${prevItem.id}&parentId=${parentId}`;
}

function nextItem() {
    if (currentIndex >= items.length - 1) return;
    const nextItem = items[currentIndex + 1];
    window.location.href = `spotters-view.php?id=${chapterId}&itemId=${nextItem.id}&parentId=${parentId}`;
}
...
function toggleBookmark() {
    ...
    fetch('/bookmark-proxy.php?module=spotters', {
        method: 'POST',
        body: formData
    })
...
function refreshBookmarkState() {
    ...
    fetch('/bookmark-proxy.php?module=spotters&action=status', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
